...
| File/Image Storage        | **Cloudinary** (configured via environment variables `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET` for image uploads)                  |
...
| Monitoring                | **UptimeRobot** (configured to ping the `/health` endpoint every 5 minutes to prevent cold starts on Render and monitor API status)                               |
